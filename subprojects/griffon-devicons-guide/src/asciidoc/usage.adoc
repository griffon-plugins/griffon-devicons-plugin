...
The `griffon-devicons-swing:{project-version}` JAR provides a new Icon class: `{api_link_devicons_icon_swing}`.
You may use this class with any Swing component that supports an Icon property.

== JavaFX

The `griffon-devicons-javafx:{project-version}` JAR provides a new Node class: `{api_link_devicons_icon_javafx}`.
